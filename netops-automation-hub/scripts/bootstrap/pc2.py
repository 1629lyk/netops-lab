...
def bootstrap(results):
    section("PC2  (VLAN20 — 10.20.0.11)")

    conn = connect(PC2_HOST, PC_USER, PC2_PASS)
    if not conn:
        results["PC2"] = {"status": "FAIL", "notes": "SSH unreachable"}
        return

    # ── 1. Interface ──────────────────────────────────────────────────────────
    console.print("  Checking eth0...")
    eth0_out = run(conn, "ip addr show eth0")
    if "10.20.0.11" not in eth0_out:
        warn("eth0 missing IP — applying...")
        run_config(conn, "ip link set eth0 mtu 1500")
        run_config(conn, "ip link set eth0 up")
        run_config(conn, "ip addr add 10.20.0.11/24 dev eth0")
        ok("eth0 IP assigned: 10.20.0.11/24")
    else:
        ok("eth0 IP present: 10.20.0.11/24")

    # ── 2. Default route ──────────────────────────────────────────────────────
    console.print("  Checking default route...")
    route_out = run(conn, "ip route show default")
    if "10.20.0.1" not in route_out:
        warn("Default route missing — applying...")
        run_config(conn, "ip route add default via 10.20.0.1")
        ok("Default route added via 10.20.0.1")
    else:
        ok("Default route present via 10.20.0.1")
    ...
    ping_out = run(conn, "ping -c4 8.8.8.8")
    if " received" in ping_out and "0 received" not in ping_out:
        ok("Internet reachable from PC2")
        results["PC2"] = {"status": "PASS", "notes": "All checks passed"}
    else:
        fail("Internet NOT reachable from PC2")
        results["PC2"] = {"status": "FAIL", "notes": "Internet ping failed"}
